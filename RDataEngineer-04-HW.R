...
twii_head <- {
  # 請填寫你的程式碼
  dbReadTable(db, "TWII")$date[1] # = head(dbReadTable(db, "TWII")$date, 1)
}
twii_tail <- {
  # 請填寫你的程式碼
...
{
  # 請填寫你的程式碼
  irisSetosa <- iris[iris$Species == "setosa", ] # 別忘了","及其意義
  dbWriteTable(db, "setosa", irisSetosa, overwrite = TRUE) # overwrite
