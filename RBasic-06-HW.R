...
#' 請問uptake這欄的平均值為多少？
answer4 <- mean(CO2$uptake) # = mean(CO2[["uptake"]]) != mean(CO2["uptake"])
...
answer5 <- CO2[CO2$uptake > answer4,] # CO2$uptake[CO2$uptake > answer4]  = CO2[["uptake"]][CO2$uptake > answer4] 是從CO2$uptake裡面挑出CO2$uptake > answer4的值的向量，跟答案要挑列是不一樣的

#' 請問Type有多少種類別？
#' ps. answer6 應該是一個整數
answer6 <- length(levels(CO2$Type)) #是問有多少種"種類"喔!!!不是Type那欄有多少(length(CO2$Type))

#' 請問當類別為Quebec時，uptake的平均值為多少？
answer7 <- mean(CO2[CO2$Type == "Quebec", "uptake"]) # = mean(CO2$uptake[CO2$Type == "Quebec"])

#' 請問當類別為Mississippi時，uptake的平均值為多少？
answer8 <- mean(CO2[CO2$Type == "Mississippi", "uptake"]) # = mean(CO2$Mississippi[CO2$Type == "Quebec"])
