...
answer02.1 <- local({
  # 請完成你的程式碼
  select(flights, year:day, hour, origin, dest, tailnum, carrier, arr_delay) %>% # 先挑出合併的key
    left_join(weather) %>% # 預設會用weather與flights的欄位名稱重複的部分 等於先做了intersect(colnames(flights), colnames(weather))
    select(wind_speed, arr_delay) %>% # 只包含wind_speed與arr_delay這兩個欄位
    filter(!is.na(wind_speed), !is.na(arr_delay)) # 這些欄位中都不應該有NA或NaN
})
